...
import { resetMe, updateMeLoadingDelete } from '#store/me/actionCreators';
...
const successDeleteMethod = (dispatch: Dispatch<Store.Action>) => {
    AsyncStorage.clear().finally(() => {
        ...
        dispatch(
            updateNotification({
                status: 'success',
                text: "you're account has been successfully deleted",
            })
        );
        dispatch(updateMeLoadingDelete('SUCCESS'));
    });
};
